...
function swap(puzzleContainer, square) {
    ...
    if (isAdjacent(gapIndex, squareIndex)) {
        ...
        if (isSolved(puzzleContainer)) {
            window.location.href = "callum11.html";
        } 
    }
}
...
function isAdjacent(gapIndex, squareIndex) {
    const row1 = Math.floor(gapIndex / 4);
    const col1 = gapIndex % 4;
    const row2 = Math.floor(squareIndex / 4);
    const col2 = squareIndex % 4;
...
function isSolved(puzzleContainer) {
    ...
    const solution = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O", ' '];
...
function redirectToAnotherPage() {
    window.location.href = "callum11.html"; // Replace with the URL of the page you want to redirect to
}
